"""1.12 花园新变化提示：GET /api/v1/badge（按当前会话的花园；查看花园 1.3 后清除）。"""
...
def get_badge(garden_id: int = Depends(resolve_garden_id), db: Session = Depends(get_db)):
    badge = db.get(Badge, garden_id)
    return BadgeOut(
